<?php 

	session_start();
	//print_r($_SESSION['tablaComprasTemp']);
 ?>
<br> <br> <br>
 
 <table class="table table-bordered table-hover table-condensed" style="text-align: center;">
 	
	 <style> .la{color: #6c2eb9;
		font-family: "Times New Roman", Times, serif;
  font-weight: normal;
  font-size: 15px;
  text-transform: uppercase; 

  } </style>
 	<tr class="la">
 		<td>Producto</td>
 		<td>Descripcion</td>
 		<td>Precio</td>
 		<td>Cliente</td>
		<td>Cantidad</td> 
 		<td>Quitar</td>
 	</tr>
 	<?php 
 	$total=0;//esta variable tendra el total de la compra en dinero
 	$cliente=""; //en esta se guarda el nombre del cliente
 		if(isset($_SESSION['tablaComprasTemp'])):
 			$i=0;
 			foreach (@$_SESSION['tablaComprasTemp'] as $key) {

 				$d=explode("||", @$key);
 	 ?>
<style> .ma{color: #008000;
		font-family: "Times New Roman", Times, serif;
  font-weight: normal;
  font-size: 15px;
     } </style>
 	<tr class="ma">
 		<td class="ma"><?php echo $d[1] ?></td>
 		<td class="ma"><?php echo $d[2] ?></td>
 		<td class="ma"><?php echo $d[3] ?></td>
		<td class="ma"><?php echo $d[4] ?></td>
 		<td class="ma"><?php echo 1; ?></td>
 		<td>
 			<span class="btn btn-danger btn-xs" onclick="quitarP('<?php echo $i; ?>')">
 				<span class="glyphicon glyphicon-remove"></span>
 			</span>
 		</td>
 	</tr>

 <?php 
 		$total=$total + $d[3];
 		$i++;
 		$cliente=$d[4];
 	}
 	endif; 
 ?>

 	<tr>
 		<td class="ma">Total de venta: <?php echo "$".$total; ?></td>
 	</tr>

 </table>
 <caption>
 		<span class="btn btn-success" onclick="crearVenta()"> Generar venta
 			<span class="glyphicon glyphicon-usd"></span>
 		</span>
 	</caption>

 <script type="text/javascript">
 	$(document).ready(function(){
 		nombre="<?php echo @$cliente ?>";
 		$('#nombreclienteVenta').text("Nombre de cliente: " + nombre);
 	});
 </script>